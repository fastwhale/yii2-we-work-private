<?php
	...
	abstract class BaseWork extends Component
	{
		...
		public $privatizedBaseUrl = '';    //todo 私有化域名
		...
		/* TODO 私有化api*/
		private $privatizedAddress
			= [
				self::USER_CREATE,
				self::USER_GET,
				self::USER_UPDATE,
				self::USER_DELETE,
				self::USER_BATCH_DELETE,
				...
				self::TAG_DELETE,
				self::TAG_GET,
				self::TAG_ADD_TAG_USERS,
				self::TAG_DEL_TAG_USERS,
				self::TAG_LIST,
				self::BATCH_SYNC_USER,
				self::BATCH_REPLACE_USER,
				self::BATCH_REPLACE_PARTY,
				self::BATCH_GET_RESULT,
				self::USR_GET_USER_INFO,
				self::USER_GET_USER_DETAIL,
				self::AGENT_GET,
				self::AGENT_LIST,
				self::AGENT_SET,
				self::MENU_CREATE,
				...
				self::MENU_DELETE,
				self::MESSAGE_SEND,
				self::APP_CHAT_CREATE,
				self::APP_CHAT_UPDATE,
				self::APP_CHAT_GET,
				self::APP_CHAT_SEND,
				self::MEDIA_UPLOAD,
				self::MEDIA_GET,
		...
		protected function _HttpCall ($url, $method = "GET", $args = [], $refreshTokenWhenExpired = true, $isPostFile = false)
		{
			if ('POST' == strtoupper($method)) {
				$url = HttpUtils::MakeUrl($url, $this->privatizedBaseUrl, $this->getPrivatizedAddress());
		...
		public function getPrivatizedAddress ()
		{
			foreach ($this->privatizedAddress as $key => $value) {
				$this->privatizedAddress[$key] = parse_url($value, PHP_URL_PATH);;
			}

			return $this->privatizedAddress;
		}
	}
